...
export default function DashboardContainer() {
  ...
  const { players, selectedPlayer, setSelectedPlayer, prevPlayersRef } = usePlayerUpdates(
    initialMockPlayers
  );
  ...
  const { isOnline, serverLoad, activePlayers } = useServerStatus(onOnlineChange);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-inter antialiased">
      ...
      <Header
        ...
        onClearAllNotificationsAndCloseDropdown={clearAllNotificationsAndCloseDropdown}
      ...
      <div className="fixed bottom-4 right-4 z-50 hidden md:block">
        <AnimatePresence>
          {displayedNotifications.map((notification) => (
            <Toast key={notification.id} notification={notification} onClose={closeToast} />
          ))}
        </AnimatePresence>
      </div>
